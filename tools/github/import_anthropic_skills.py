...
def _http_json(
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": "MetaAgentTools/1.0 (import_anthropic_skills.py)",
    }
    if token:
...
def _http_text(
    ...
    for attempt in range(retries):
        ...
        req.add_header("User-Agent", "MetaAgentTools/1.0 (import_anthropic_skills.py)")
...
def _ai_generate_description(
    ...
    user_prompt = textwrap.dedent(
        f"""
        Write a short Chinese description for an MetaAgent Skill Market listing.
...
def _build_metaagent_issue_body(description: str, repository_url: str, version: str) -> str:
    metadata = {
    ...
    lines: list[str] = []
    lines.append(f"<!-- metaagent-skill-json: {json.dumps(metadata, ensure_ascii=False, separators=(',', ':'))} -->")
    lines.append(f"<!-- metaagent-parser-version: {version} -->")
    lines.append("")
    ...
    lines.append("## 📦 安装方式")
    lines.append("")
    lines.append("1. 打开 MetaAgent → 包管理 → Skills")
    ...
    lines.append("| 项目 | 值 |")
    lines.append("|------|-----|")
    lines.append("| 发布平台 | MetaAgent Skill 市场 |")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Import skills from awesome-agent-skills into MetaAgent Skill Market by creating GitHub issues."
    )
    parser.add_argument(
        "--env",
    ...
    parser.add_argument(
        "--target-repo",
        default="AAswordman/MetaAgentSkillMarket",
    ...
    if args.dry_run:
        ...
        def _prepare_dry_run_item(item_index: int, skill: SourceSkill) -> dict:
            ...
            body = _build_metaagent_issue_body(description=description, repository_url=repo_url, version=issue_version)
            return {
                "ok": True,
                "skip": False,
                "index": item_index,
