#!/usr/bin/env python3
"""Package tools/metaagent-pc-agent into windows_control toolpkg resources as a deterministic ZIP.

Default output:
    examples/windows_control/resources/pc_agent/metaagent-pc-agent.zip
"""
...
DEFAULT_ZIP_ROOT = "metaagent-pc-agent"
...
def parse_args() -> argparse.Namespace:
    ...
    default_source = repo_root / "tools" / "metaagent-pc-agent"
    default_output = repo_root / "examples" / "windows_control" / "resources" / "pc_agent" / "metaagent-pc-agent.zip"

    parser = argparse.ArgumentParser(description="Package metaagent-pc-agent into windows_control toolpkg resources.")
    parser.add_argument(
        "--source",
        type=Path,
...
def main() -> int:
    ...
    print("[OK] Packaged metaagent-pc-agent asset")
