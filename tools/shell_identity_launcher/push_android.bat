...
set "BINARY=%SCRIPT_DIR%\build\metaagent_shell_exec"
set "DEVICE_PATH=/data/local/tmp/metaagent_shell_exec"
...
echo.
echo [OK] Deployed metaagent_shell_exec to %DEVICE_PATH%

echo Running self-test via metaagent_shell_exec ...
...
echo [INFO] Starting Shower server via metaagent_shell_exec (native launcher) ...
adb shell su -c "%DEVICE_PATH% CLASSPATH=%REMOTE_CLASSPATH% app_process / com.ai.assistance.shower.Main"
if errorlevel 1 (
    echo [WARN] Failed to start Shower server via metaagent_shell_exec; please check manually.
) else (
