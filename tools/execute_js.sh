...
TARGET_DIR="/sdcard/Android/data/com.ai.assistance.metaagent/js_temp"
...
if [ "$HAS_ENV_FILE" = "true" ]; then
    ...
    adb -s "$DEVICE_SERIAL" shell "am broadcast -a com.ai.assistance.metaagent.EXECUTE_JS -n com.ai.assistance.metaagent/.core.tools.javascript.ScriptExecutionReceiver --include-stopped-packages --es file_path '$TARGET_FILE' --es function_name '$FUNCTION_NAME' --es params '$ESCAPED_PARAMS' --ez temp_file true"
fi
