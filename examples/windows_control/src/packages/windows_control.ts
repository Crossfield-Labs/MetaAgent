/* METADATA
{
    ...
    "description": {
        "zh": "通过 HTTP 调用 MetaAgent PC Agent 控制 Windows 电脑，支持执行 PowerShell/CMD 命令并返回输出。",
        "en": "Control a Windows PC through MetaAgent PC Agent over HTTP, execute PowerShell/CMD commands, and return output."
    },
    ...
    "env": [
        {
            ...
            "description": { "zh": "MetaAgent PC Agent 地址，例如 http://192.168.1.8:58321", "en": "MetaAgent PC Agent URL, e.g. http://192.168.1.8:58321" },
            "required": true
        },
        {
    ...
    "tools": [
        ...
        },
        {
            "name": "windows_exec",
            "description": {
                "zh": "通过 MetaAgent PC Agent 的 HTTP 接口在 Windows 上执行命令。",
                "en": "Execute commands on Windows via MetaAgent PC Agent HTTP API."
            },
            "parameters": [
                {
                    "name": "command",
...
const windowsControl = (function () {
    ...
    function buildVersionMismatchMessage(remoteVersion: string): string {
        return [
            ...
            "请前往 Windows 一键配置，重新上传最新 metaagent-pc-agent.zip 到电脑并运行，然后再粘贴最新配置。"
        ].join(" ");
    }
    ...
    async function persistWindowsExecOutputIfTooLong(
        ...
        return {
            ...
            outputChars,
            metaagentCleanOnExitDir: OPERIT_CLEAN_ON_EXIT_DIR,
