...
```kotlin
class MetaAgentShowerShellRunner : ShellRunner {
...
- 运行时库会自动从自身 `assets` 中读取，并复制到 `/sdcard/Download/MetaAgent/shower-server.jar`，再拷贝到 `/data/local/tmp/shower-server.jar`。
...
广播协议（与主项目保持一致）：
 ...
 - **Action**：`com.ai.assistance.metaagent.action.SHOWER_BINDER_READY`
...
class ShowerBinderReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val ACTION_SHOWER_BINDER_READY =
            "com.ai.assistance.metaagent.action.SHOWER_BINDER_READY"
...
<receiver
    ...
    <intent-filter>
        <action android:name="com.ai.assistance.metaagent.action.SHOWER_BINDER_READY" />
    </intent-filter>
</receiver>
```
