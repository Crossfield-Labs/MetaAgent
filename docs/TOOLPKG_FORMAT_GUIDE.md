...
**ToolPkg** 是 MetaAgent 项目中用于打包和分发工具包的标准格式。它允许开发者将多个相关的工具脚本、资源文件和 UI 模块打包成一个单一的、易于分发和管理的文件。
...
{
  "schema_version": 1,
  "toolpkg_id": "com.metaagent.windows_bundle",
  "version": "0.2.0",
  "main": "main.js",
  "display_name": {
    "zh": "Windows 工具包",
  ...
  "subpackages": [
    {
      ...
      "description": {
        "zh": "通过 MetaAgent PC Agent 控制 Windows",
        "en": "Control Windows via MetaAgent PC Agent"
      }
    }
  ],
  "resources": [
    {
      "key": "pc_agent_zip",
      "path": "resources/pc_agent/metaagent-pc-agent.zip",
...
| `toolpkg_id` | string | 是 | 包的唯一标识符，建议使用反向域名格式（如 `com.metaagent.windows_bundle`） |
...
/* METADATA
{
    ...
    "description": {
        "zh": "通过 HTTP 调用 MetaAgent PC Agent 控制 Windows 电脑",
        "en": "Control a Windows PC through MetaAgent PC Agent over HTTP"
    },
    ...
    "env": [
        {
            ...
            "description": {
                "zh": "MetaAgent PC Agent 地址",
                "en": "MetaAgent PC Agent URL"
            },
            "required": true
        }
    ],
...
用户可以通过以下方式导入外部包：
1. 将 `.toolpkg` 文件复制到设备的 `Android/data/com.ai.assistance.metaagent/files/packages/` 目录
2. 在应用中使用"导入包"功能
...
- **toolpkg_id**：使用反向域名格式，如 `com.metaagent.windows_bundle`
...
- [PackageManager.kt](../app/src/main/java/com/ai/assistance/metaagent/core/tools/packTool/PackageManager.kt)：包管理器源码
- [ToolPkgParser.kt](../app/src/main/java/com/ai/assistance/metaagent/core/tools/packTool/ToolPkgParser.kt)：解析器源码
- [JsComposeDslBridge.kt](../app/src/main/java/com/ai/assistance/metaagent/core/tools/javascript/JsComposeDslBridge.kt)：Compose DSL 桥接
