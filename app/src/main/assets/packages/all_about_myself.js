/* METADATA
{
  ...
  display_name: {
    zh: "MetaAgent配置编辑器"
    en: "MetaAgent Config Editor"
  }
  description: {
    zh: '''软件设置直改工具包：提供一组可直接读取与修改 MetaAgent 设置的工具，覆盖 MCP、Skill、Sandbox Package、功能模型绑定、模型参数、上下文总结与 TTS/STT 语音服务配置。'''
    en: '''Direct software-settings toolkit: a collection of tools for reading and directly modifying MetaAgent settings, covering MCP, Skill, Sandbox Package, function-model bindings, model parameters, context-summary settings, and TTS/STT speech-service configuration.'''
  }
...
- 用户让你排查 MetaAgent 的插件配置路径、部署目录、开关状态、环境变量
...
【MCP：安装与排查】
0) 市场来源：
- OPR MCP 市场（issues）：https://github.com/AAswordman/MetaAgentMCPMarket/issues
- 该市场内容较少，可在其他可信来源继续搜集 MCP，再按本手册部署。
1) 配置目录：/sdcard/Download/MetaAgent/mcp_plugins/
- 主配置：mcp_config.json
...
【Skill：安装与排查】
0) 市场来源：
- OPR Skill 市场（issues）：https://github.com/AAswordman/MetaAgentSkillMarket/issues
- 也可从其他可信来源获取 skill，下载后解压到 skills 目录。
1) 目录：/sdcard/Download/MetaAgent/skills/
2) 识别规则：每个 Skill 必须是一个文件夹，且包含 SKILL.md（skill.md 也可）。
3) 添加方式（按这个做）：
- 先从可信来源下载 Skill（zip 或仓库源码均可）
- 把下载内容解压后，直接放到 /sdcard/Download/MetaAgent/skills/
- 最终目录结构必须是 /sdcard/Download/MetaAgent/skills/<skill_name>/，且该目录内有 SKILL.md
4) 元数据解析：
...
1) 沙盒包目录（外部）：
- Android/data/com.ai.assistance.metaagent/files/packages
2) 内置包：
...
0) Market source:
- OPR MCP market (issues): https://github.com/AAswordman/MetaAgentMCPMarket/issues
- This market is still small; you can continue collecting MCPs from other trusted sources, then deploy with this guide.
1) Config directory: /sdcard/Download/MetaAgent/mcp_plugins/
...
0) Market source:
- OPR Skill market (issues): https://github.com/AAswordman/MetaAgentSkillMarket/issues
- You can also get skills from other trusted sources, then download and extract them into the skills directory.
1) Directory: /sdcard/Download/MetaAgent/skills/
...
- Extract it, then place the folder directly under /sdcard/Download/MetaAgent/skills/
- Final structure must be /sdcard/Download/MetaAgent/skills/<skill_name>/ and that folder must contain SKILL.md
4) Metadata parsing:
...
1. 先创建目录：/sdcard/Download/MetaAgent/skills/<skill_name>/
2. 必备文件：SKILL.md
...
1. Create a directory: /sdcard/Download/MetaAgent/skills/<skill_name>/
