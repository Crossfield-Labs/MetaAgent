package com.ai.assistance.metaagent.data.mcp
...
class MCPRepository(private val context: Context) {
    ...
    private val pluginsBaseDir by lazy {
        ...
        val metaagentDir = File(downloadsDir, OPERIT_DIR_NAME)
        val pluginsDir = File(metaagentDir, PLUGINS_DIR_NAME)

        if (!metaagentDir.exists()) metaagentDir.mkdirs()
    ...
    suspend fun syncBridgeStatus() {
        withContext(Dispatchers.IO) {
            ...
            try {
                val bridge = com.ai.assistance.metaagent.data.mcp.plugins.MCPBridge.getInstance(context)
