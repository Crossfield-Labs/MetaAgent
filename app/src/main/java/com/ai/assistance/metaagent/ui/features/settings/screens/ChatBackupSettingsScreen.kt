...
import com.ai.assistance.metaagent.data.model.PreferenceProfile
import com.ai.assistance.metaagent.data.backup.MetaAgentBackupDirs
import com.ai.assistance.metaagent.data.backup.RawSnapshotBackupManager
import com.ai.assistance.metaagent.data.backup.RoomDatabaseBackupManager
import com.ai.assistance.metaagent.data.backup.RoomDatabaseBackupPreferences
import com.ai.assistance.metaagent.data.backup.RoomDatabaseBackupScheduler
import com.ai.assistance.metaagent.data.backup.RoomDatabaseRestoreManager
...
import com.ai.assistance.metaagent.data.converter.ExportFormat
import com.ai.assistance.metaagent.data.converter.ChatFormat
import com.ai.assistance.metaagent.ui.features.settings.components.BackupFilesStatisticsCard
import com.ai.assistance.metaagent.ui.features.settings.components.CharacterCardManagementCard
import com.ai.assistance.metaagent.ui.features.settings.components.ChatHistoryOperation
import com.ai.assistance.metaagent.ui.features.settings.components.DataManagementCard
import com.ai.assistance.metaagent.ui.features.settings.components.DeleteConfirmationDialog
import com.ai.assistance.metaagent.ui.features.settings.components.ExportFormatDialog
import com.ai.assistance.metaagent.ui.features.settings.components.FaqCard
import com.ai.assistance.metaagent.ui.features.settings.components.ImportFormatDialog
import com.ai.assistance.metaagent.ui.features.settings.components.MemoryImportStrategyDialog
import com.ai.assistance.metaagent.ui.features.settings.components.MemoryManagementCard
import com.ai.assistance.metaagent.ui.features.settings.components.MemoryOperation
import com.ai.assistance.metaagent.ui.features.settings.components.ModelConfigExportWarningDialog
import com.ai.assistance.metaagent.ui.features.settings.components.ModelConfigManagementCard
import com.ai.assistance.metaagent.ui.features.settings.components.ModelConfigOperation
import com.ai.assistance.metaagent.ui.features.settings.components.ManagementButton
import com.ai.assistance.metaagent.ui.features.settings.components.OperationProgressView
import com.ai.assistance.metaagent.ui.features.settings.components.OperationResultCard
import com.ai.assistance.metaagent.ui.features.settings.components.OverviewCard
import com.ai.assistance.metaagent.ui.features.settings.components.ProfileSelectionDialog
import com.ai.assistance.metaagent.ui.features.settings.components.RoomDbBackupListItem
import com.ai.assistance.metaagent.ui.features.settings.components.SectionHeader
import com.ai.assistance.metaagent.ui.features.settings.components.CharacterCardOperation
import com.ai.assistance.metaagent.ui.main.MainActivity
import java.io.File
...
fun ChatBackupSettingsScreen() {
    ...
    // 扫描 MetaAgent 目录中的备份文件
    LaunchedEffect(Unit) {
        scope.launch {
            isScanning = true
            try {
                val legacyDir = MetaAgentBackupDirs.metaagentRootDir()
                ...
                val chatFiles = mergedFiles(MetaAgentBackupDirs.chatDir())
                val characterCardFiles = mergedFiles(MetaAgentBackupDirs.characterCardsDir())
                val memoryFiles = mergedFiles(MetaAgentBackupDirs.memoryDir())
                val modelConfigFiles = mergedFiles(MetaAgentBackupDirs.modelConfigDir())
                val roomDbFiles = mergedFiles(MetaAgentBackupDirs.roomDbDir())
                ...
                onExport = {
                    scope.launch {
                        ...
                        try {
                            ...
                            val exportDir = MetaAgentBackupDirs.modelConfigDir()
                                ...
                                onClick = {
                                    ...
                                    scope.launch {
                                        ...
                                        try {
                                            val legacyDir = MetaAgentBackupDirs.metaagentRootDir()
                                            val legacyFiles = legacyDir.listFiles()?.toList() ?: emptyList()
                                            val newFiles = MetaAgentBackupDirs.roomDbDir().listFiles()?.toList() ?: emptyList()
                        ...
                        try {
                            ...
                            operationMessage = if (importResult.total > 0) {
                                ...
                                val formatName = when (selectedImportFormat) {
                                    ChatFormat.OPERIT -> context.getString(R.string.backup_format_metaagent)
...
private suspend fun exportMemories(_context: Context, memoryRepository: MemoryRepository): String? =
    withContext(Dispatchers.IO) {
        try {
            ...
            val exportDir = MetaAgentBackupDirs.memoryDir()
