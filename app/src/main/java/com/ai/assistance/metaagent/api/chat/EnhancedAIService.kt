package com.ai.assistance.metaagent.api.chat

import android.content.Context
import android.content.Intent
import android.os.Build
import com.ai.assistance.metaagent.util.AppLogger
import com.ai.assistance.metaagent.api.chat.enhance.ConversationMarkupManager
import com.ai.assistance.metaagent.api.chat.enhance.ConversationRoundManager
import com.ai.assistance.metaagent.api.chat.enhance.ConversationService
import com.ai.assistance.metaagent.api.chat.enhance.FileBindingService
import com.ai.assistance.metaagent.api.chat.enhance.InputProcessor
import com.ai.assistance.metaagent.api.chat.enhance.MultiServiceManager
import com.ai.assistance.metaagent.api.chat.enhance.ToolExecutionManager
import com.ai.assistance.metaagent.api.chat.llmprovider.AIService
import com.ai.assistance.metaagent.core.chat.hooks.PromptHookContext
import com.ai.assistance.metaagent.core.chat.hooks.PromptHookRegistry
import com.ai.assistance.metaagent.core.chat.hooks.toPromptMessages
import com.ai.assistance.metaagent.core.chat.hooks.toRoleContentPairs
import com.ai.assistance.metaagent.core.application.ActivityLifecycleManager
...
import com.ai.assistance.metaagent.data.model.ToolInvocation
...
import com.ai.assistance.metaagent.util.stream.StreamCollector
import com.ai.assistance.metaagent.util.stream.plugins.StreamXmlPlugin
import com.ai.assistance.metaagent.util.stream.splitBy
import com.ai.assistance.metaagent.util.stream.stream
...
import com.ai.assistance.metaagent.data.repository.CustomEmojiRepository
...
import com.ai.assistance.metaagent.core.config.SystemToolPrompts
import com.ai.assistance.metaagent.data.model.ToolPrompt
import com.ai.assistance.metaagent.data.model.ToolParameterSchema
import com.ai.assistance.metaagent.util.ChatUtils
...
class EnhancedAIService private constructor(private val context: Context) {
     ...
     * @param newState 新的状态
     */
    fun setInputProcessingState(newState: com.ai.assistance.metaagent.data.model.InputProcessingState) {
    ...
    init {
        com.ai.assistance.metaagent.api.chat.library.ProblemLibrary.initialize(context)
        initScope.launch {
            runCatching {
                ensureInitialized()
            }.onFailure { e ->
    ...
    private suspend fun getModelParametersForFunction(
        ...
        chatModelConfigIdOverride: String? = null,
        chatModelIndexOverride: Int? = null
    ): List<com.ai.assistance.metaagent.data.model.ModelParameter<*>> {
        ensureInitialized()
        val overrideConfigId = chatModelConfigIdOverride?.takeIf { it.isNotBlank() }
        return if (functionType == FunctionType.CHAT && overrideConfigId != null) {
    ...
    private suspend fun handleTaskCompletion(context: MessageExecutionContext, content: String, enableMemoryQuery: Boolean, isSubTask: Boolean, characterName: String? = null, avatarUri: String? = null) {
        ...
        if (enableMemoryQuery) {
            ...
            toolProcessingScope.launch {
                com.ai.assistance.metaagent.api.chat.library.ProblemLibrary.saveProblemAsync(
                        this@EnhancedAIService.context,
                        toolHandler,
    ...
    private fun serializePromptHookModelParameters(
        modelParameters: List<com.ai.assistance.metaagent.data.model.ModelParameter<*>>
    ): List<Map<String, Any?>> {
