...
/**
 * App-wide logger with an API closely mirroring [com.ai.assistance.metaagent.util.AppLogger].
 *
...
object AppLogger {
    ...
    // Mirror com.ai.assistance.metaagent.util.AppLogger priority constants
    ...
    private const val LOG_FILE_NAME = "metaagent.log"
    ...
    private fun resolveLogFile(): File? {
        ...
        return try {
            val appContext: Context = try {
                MetaAgentApplication.instance.applicationContext
            } catch (_: Throwable) {
    ...
    private fun resolvePackageLogFile(): File? {
        ...
        return try {
            ...
            val dir = File(MetaAgentPaths.metaagentRootDir(), PACKAGE_LOG_DIR_NAME)
            if (!dir.exists()) {
                dir.mkdirs()
            }
            val startupMs = MetaAgentApplication.appStartupTimeMs.takeIf { it > 0L } ?: System.currentTimeMillis()
    ...
    // --- Public API mirroring com.ai.assistance.metaagent.util.AppLogger ---
    ...
    fun resetLogFile() {
        try {
            val appContext: Context = MetaAgentApplication.instance.applicationContext
            val dir = File(appContext.filesDir, LOG_DIR_NAME)
