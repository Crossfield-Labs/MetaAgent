...
MetaAgent 当前脚本运行时不是 Node.js，也不是浏览器原生 ESM 运行时。
...
```js
const SomeBridge = Java.com.ai.assistance.metaagent.SomeBridge;

const resultA = await SomeBridge.callSuspend('loadSomething', 'arg1');

const resultB = await Java.callSuspend(
  'com.ai.assistance.metaagent.SomeBridge',
  'loadSomething',
  'arg1'
);
```
...
const ActivityLifecycleManager = Java.com.ai.assistance.metaagent.core.application.ActivityLifecycleManager;
...
```js
Java.loadDex('/data/user/0/com.ai.assistance.metaagent/files/plugins/demo.dex');
...
```js
Java.loadJar('/data/user/0/com.ai.assistance.metaagent/files/plugins/demo.jar');
...
```js
Java.loadDex('/data/user/0/com.ai.assistance.metaagent/files/plugins/demo.dex', {
  nativeLibraryDir: '/data/user/0/com.ai.assistance.metaagent/files/plugins/lib'
});
```
...
```js
const imported = await ctx.isPackageImported('com.metaagent.xxx');
if (!imported) {
  await ctx.importPackage('com.metaagent.xxx');
}
```
