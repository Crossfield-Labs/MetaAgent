package com.ai.assistance.metaagent.core.tools.defaultTool.debugger
...
import com.ai.assistance.metaagent.core.tools.defaultTool.accessbility.AccessibilityFileSystemTools
...
open class DebuggerFileSystemTools(context: Context) : AccessibilityFileSystemTools(context) {
    ...
    companion object {
        ...
        private const val OPERIT_PACKAGE = "com.ai.assistance.metaagent"

        internal fun isMetaAgentInternalPath(path: String): Boolean {
    ...
    /**
     * 判断路径是否为MetaAgent应用的内部存储路径
     * 仅针对 /data/data/com.ai.assistance.metaagent 开头的路径返回true
     */
    protected fun isMetaAgentInternalPath(path: String): Boolean {
        return Companion.isMetaAgentInternalPath(path)
    }
    ...
    override suspend fun listFiles(tool: AITool): ToolResult {
        ...
        // 如果是MetaAgent内部存储路径，使用super（AccessibilityFileSystemTools）的高权限方法
        if (isMetaAgentInternalPath(path)) {
            return super.listFiles(tool)
        }
    ...
    override suspend fun readFileFull(tool: AITool): ToolResult {
        ...
        // 如果是MetaAgent内部存储路径，使用super的高权限方法
        if (isMetaAgentInternalPath(path)) {
            return super.readFileFull(tool)
        }
    ...
    override suspend fun moveFile(tool: AITool): ToolResult {
        ...
        // 如果源文件或目标文件在MetaAgent内部存储，使用super的高权限方法
        if (isMetaAgentInternalPath(sourcePath) || isMetaAgentInternalPath(destPath)) {
            return super.moveFile(tool)
        }
    ...
    override suspend fun zipFiles(tool: AITool): ToolResult {
        ...
        if (isMetaAgentInternalPath(sourcePath) || isMetaAgentInternalPath(zipPath)) {
            return super.zipFiles(tool)
        }
    ...
    override suspend fun unzipFiles(tool: AITool): ToolResult {
        ...
        if (isMetaAgentInternalPath(zipPath) || isMetaAgentInternalPath(destPath)) {
            return super.unzipFiles(tool)
        }
