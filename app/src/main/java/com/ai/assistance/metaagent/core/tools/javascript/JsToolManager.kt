...
class JsToolManager private constructor(
    ...
    private fun buildRuntimeParams(
        ...
        packageManager.getActivePackageStateId(packageName)?.let {
            runtimeParams["__metaagent_package_state"] = it
        }

        listOf(
            "__metaagent_package_caller_name",
            "__metaagent_package_chat_id",
            "__metaagent_package_caller_card_id"
        ).forEach { key ->
        ...
        runtimeParams["__metaagent_package_name"] = packageName

        packageManager.resolveToolPkgSubpackageRuntimeInternal(packageName)?.let { runtime ->
            runtimeParams["__metaagent_toolpkg_subpackage_id"] = runtime.subpackageId
            runtimeParams["containerPackageName"] = runtime.containerPackageName
            runtimeParams["toolPkgId"] = runtime.containerPackageName
            runtimeParams["__metaagent_ui_package_name"] = runtime.containerPackageName
        } ?: run {
            runtimeParams.remove("__metaagent_toolpkg_subpackage_id")
            runtimeParams.remove("containerPackageName")
            runtimeParams.remove("toolPkgId")
            runtimeParams.remove("__metaagent_ui_package_name")
        }
    ...
    fun executeComposeDsl(
        ...
        if (packageName.isNotBlank()) {
            packageManager.getActivePackageStateId(packageName)?.let {
                runtimeOptions["__metaagent_package_state"] = it
            }
        }
