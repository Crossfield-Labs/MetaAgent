...
class JsEngine(private val context: Context) {
    ...
    private val quickJsExecutor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "MetaAgentQuickJsEngine").apply {
            isDaemon = true
            quickJsThread = this
        }
    }
    ...
    @Volatile
    private var quickJs: MetaAgentQuickJsEngine? = null
    ...
    private fun nextExecutionCallId(): String {
        return "metaagent_call_${UUID.randomUUID().toString().replace("-", "")}" 
    }
            ...
            script =
                """
                    (function() {
                        ...
                        if (typeof root.__metaagentCancelCallSession === 'function') {
                            root.__metaagentCancelCallSession($safeCallId, $safeReason);
                        }
                    })();
                """.trimIndent(),
    ...
    private fun runtimeBootstrapModules(): List<JsBootstrapModule> {
        return buildRuntimeBootstrapModules(
            context = context,
            metaagentDownloadDir = MetaAgentPaths.metaagentRootPathSdcard(),
            metaagentCleanOnExitDir = MetaAgentPaths.cleanOnExitPathSdcard()
        )
    }
        ...
        return """
            (function() {
                var root = typeof globalThis !== 'undefined'
                    ? globalThis
                    : (typeof window !== 'undefined' ? window : this);
                var expose = typeof root.__metaagentExpose === 'function'
                    ? root.__metaagentExpose
                    : function(name, value) {
    ...
    private fun invokeJavaBridgeJsObjectCallbackSync(
        ...
        val callbackScript =
            ...
            (function() {
                try {
                    var __invoker =
                        (typeof globalThis !== 'undefined' && typeof globalThis.__metaagentJavaBridgeInvokeJsObject === 'function')
                            ? globalThis.__metaagentJavaBridgeInvokeJsObject
                            : undefined;
                    if (!__invoker) {
    ...
    private fun releaseJavaBridgeJsObjectSync(jsObjectId: String): Boolean {
        ...
        val releaseScript =
            ...
            (function() {
                try {
                    var __release =
                        (typeof globalThis !== 'undefined' && typeof globalThis.__metaagentJavaBridgeReleaseJsObject === 'function')
                            ? globalThis.__metaagentJavaBridgeReleaseJsObject
                            : undefined;
                    if (!__release) {
                        return false;
                    }
    ...
    fun executeScriptFunction(
        ...
        val explicitLanguage = effectiveParams["__metaagent_package_lang"]?.toString()?.trim().orEmpty()
        if (explicitLanguage.isBlank()) {
            effectiveParams["__metaagent_package_lang"] =
        ...
        val timingPluginId =
            effectiveParams["pluginId"]?.toString()?.trim().orEmpty()
                .ifBlank {
                    effectiveParams["__metaagent_plugin_id"]?.toString()?.trim().orEmpty()
                }
                .ifBlank { "none" }
    ...
    fun executeComposeDslScript(
        ...
        return executeScriptFunction(
                ...
                functionName = "__metaagent_render_compose_dsl",
    ...
    fun executeComposeDslAction(
        ...
        return executeScriptFunction(
                script = "",
                functionName = "__metaagent_dispatch_compose_dsl_action",
                params = params,
                envOverrides = envOverrides,
                onIntermediateResult = onIntermediateResult
        )
                    ...
                    """
                        (function() {
                            var root = typeof globalThis !== 'undefined'
                                ? globalThis
                                : (typeof window !== 'undefined' ? window : this);
                            if (typeof root.__metaagentClearAllTimers === 'function') {
                                root.__metaagentClearAllTimers();
                            }
                        })();
                    """.trimIndent(),
