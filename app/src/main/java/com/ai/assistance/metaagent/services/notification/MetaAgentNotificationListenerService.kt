package com.ai.assistance.metaagent.services.notification
...
object MetaAgentNotificationStore {
...
class MetaAgentNotificationListenerService : NotificationListenerService() {
    override fun onListenerConnected() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                activeNotifications?.forEach { MetaAgentNotificationStore.upsert(it) }
            } catch (_: Exception) {
            }
        }
    }
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        MetaAgentNotificationStore.upsert(sbn)
    }

    override fun onNotificationRemoved(sbn: StatusBarNotification) {
        MetaAgentNotificationStore.remove(sbn)
    }
}
