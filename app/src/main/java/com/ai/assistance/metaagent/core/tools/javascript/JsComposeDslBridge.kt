...
internal fun buildComposeDslContextBridgeDefinition(): String {
    return """
        var MetaAgentComposeDslRuntime = (function() {
            ...
            function __metaagent_define_unit_getter(unitName) {
                try {
            ...
            function __metaagent_define_array_unit_getter(unitName) {
                try {
                    if (!unitName || typeof unitName !== 'string') {
                        return;
                    }
            ...
            __metaagent_define_unit_getter('px');
            __metaagent_define_unit_getter('dp');
            __metaagent_define_unit_getter('fraction');
            __metaagent_define_array_unit_getter('px');
            __metaagent_define_array_unit_getter('dp');
            __metaagent_define_array_unit_getter('fraction');
                ...
                var runtime = {
                    ...
                    moduleSpec:
                        options.moduleSpec && typeof options.moduleSpec === 'object'
                            ? options.moduleSpec
                            : {},
                    packageName: String(options.packageName || options.__metaagent_ui_package_name || ''),
                    toolPkgId: String(options.toolPkgId || options.__metaagent_ui_toolpkg_id || ''),
                    uiModuleId: String(options.uiModuleId || options.__metaagent_ui_module_id || ''),
                    callRuntime:
                        options.__metaagent_call_runtime && typeof options.__metaagent_call_runtime === 'object'
                            ? options.__metaagent_call_runtime
                            : null,
                    actionStore: {},
                    actionCounter: 0,
