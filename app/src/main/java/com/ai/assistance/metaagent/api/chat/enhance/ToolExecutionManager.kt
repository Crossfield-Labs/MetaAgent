...
import com.ai.assistance.metaagent.core.tools.ToolExecutor
...
import com.ai.assistance.metaagent.ui.common.displays.MessageContentParser
                ...
                permittedInvocations.map { invocation ->
                    ...
                    if (!isJsPackageTool) {
                        ...
                        if (!callerName.isNullOrBlank()) {
                            val hasCallerParam = updatedParams.any { it.name == "__metaagent_package_caller_name" }
                            if (!hasCallerParam) {
                                updatedParams.add(ToolParameter("__metaagent_package_caller_name", callerName))
                            }
                        }
                        if (!callerChatId.isNullOrBlank()) {
                            val hasChatIdParam = updatedParams.any { it.name == "__metaagent_package_chat_id" }
                            if (!hasChatIdParam) {
                                updatedParams.add(ToolParameter("__metaagent_package_chat_id", callerChatId))
                            }
                        }
                        if (!callerCardId.isNullOrBlank()) {
                            val hasCallerCardParam = updatedParams.any { it.name == "__metaagent_package_caller_card_id" }
                            if (!hasCallerCardParam) {
                                updatedParams.add(ToolParameter("__metaagent_package_caller_card_id", callerCardId))
                            }
                        }
                        invocation.copy(
