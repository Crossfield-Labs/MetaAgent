package com.ai.assistance.metaagent.data.preferences
...
import com.ai.assistance.metaagent.data.model.CharacterCard
import com.ai.assistance.metaagent.data.model.CharacterCardChatModelBindingMode
import com.ai.assistance.metaagent.data.model.PromptTag
import com.ai.assistance.metaagent.data.model.TagType
import com.ai.assistance.metaagent.data.model.TavernCharacterCard
import com.ai.assistance.metaagent.data.model.TavernCharacterData
import com.ai.assistance.metaagent.R
import com.ai.assistance.metaagent.data.model.TavernExtensions
import com.ai.assistance.metaagent.data.model.MetaAgentTavernExtension
import com.ai.assistance.metaagent.data.model.MetaAgentAttachedTagPayload
import com.ai.assistance.metaagent.data.model.MetaAgentCharacterCardPayload
...
class CharacterCardManager private constructor(private val context: Context) {
    ...
    suspend fun initializeIfNeeded() {
        ...
        if (isInitialized) {
            ...
            userPreferencesManager.saveAiAvatarForCharacterCard(DEFAULT_CHARACTER_CARD_ID, "file:///android_asset/metaagent.png")
        }
    ...
    data class CharacterCardsBackupFile(
        val schema: String = "metaagent_character_cards_backup_v1",
        val version: Int = 1,
    ...
    suspend fun exportAllCharacterCardsToBackupFile(): String? {
        return try {
            ...
            val exportDir = MetaAgentBackupDirs.characterCardsDir()
    ...
    suspend fun createCharacterCardFromTavernJson(jsonString: String): Result<String> {
        return try {
            ...
            val metaagentPayload = tavernCard.data.extensions?.metaagent
                ?.takeIf { it.schema == "metaagent_character_card_v1" }
                ?.character_card

            val importedMetaAgentTags = if (metaagentPayload != null) {
                importOrReuseMetaAgentTags(metaagentPayload.attachedTags)
            } else {
                ImportedTagResult(emptyMap(), emptyList())
            }

            val characterCard = if (metaagentPayload != null) {
                CharacterCard(
                    id = "",
                    name = metaagentPayload.name,
                    description = metaagentPayload.description,
                    characterSetting = metaagentPayload.characterSetting,
                    openingStatement = metaagentPayload.openingStatement,
                    otherContentChat = metaagentPayload.otherContentChat.ifBlank { metaagentPayload.otherContent },
                    otherContentVoice = metaagentPayload.otherContentVoice,
                    attachedTagIds = if (metaagentPayload.attachedTagIds.isNotEmpty()) {
                        remapAttachedTagIds(
                            sourceIds = metaagentPayload.attachedTagIds,
                            idMap = importedMetaAgentTags.idMap
                        )
                    } else {
                        if (importedMetaAgentTags.importedIds.isNotEmpty()) {
                            importedMetaAgentTags.importedIds
                        } else {
                            importedMetaAgentTags.idMap.values.distinct()
                        }
                    },
                    advancedCustomPrompt = metaagentPayload.advancedCustomPrompt,
                    marks = metaagentPayload.marks,
                    chatModelBindingMode = CharacterCardChatModelBindingMode.normalize(metaagentPayload.chatModelBindingMode),
                    chatModelConfigId = metaagentPayload.chatModelConfigId?.takeIf { it.isNotBlank() },
                    chatModelIndex = metaagentPayload.chatModelIndex.coerceAtLeast(0),
                    isDefault = false,
    ...
    suspend fun exportCharacterCardToTavernJson(characterCardId: String): Result<String> {
        return try {
            ...
            val metaagentExt = MetaAgentTavernExtension(
                character_card = MetaAgentCharacterCardPayload(
                    name = card.name,
            ...
            val tavernCard = TavernCharacterCard(
                ...
                data = TavernCharacterData(
                    ...
                    creator = "",
                    character_version = "",
                    extensions = TavernExtensions(metaagent = metaagentExt),
                    character_book = null
                )
            )
    ...
    private suspend fun importOrReuseMetaAgentTags(exportedTags: List<MetaAgentAttachedTagPayload>): ImportedTagResult {
        val idMap = mutableMapOf<String, String>()
