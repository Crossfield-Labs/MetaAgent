package com.ai.assistance.metaagent.ui.features.chat.webview
...
/**
 * 获取工作区路径（新位置：内部存储）
 * 路径: /data/data/com.ai.assistance.metaagent/files/workspace/{chatId}
 */
...
/**
 * 获取旧的工作区路径（外部存储）
 * 路径: /sdcard/Download/MetaAgent/workspace/{chatId}
 */
fun getLegacyWorkspacePath(chatId: String): String {
    return MetaAgentPaths.workspacePathSdcard(chatId)
}
...
private fun createProjectConfigIfNeeded(context: Context, workspaceDir: File, projectType: ProjectType) {
    // 创建 .metaagent 目录和 config.json
    val metaagentDir = File(workspaceDir, ".metaagent")
    if (!metaagentDir.exists()) {
        metaagentDir.mkdirs()
    }

    val configFile = File(metaagentDir, "config.json")
