...
object RoomDatabaseBackupManager {
    ...
    private suspend fun createOrReplaceAutoBackup(context: Context, day: String): File {
        ...
        val metaagentDir = MetaAgentBackupDirs.roomDbDir()

        val targetFile = File(metaagentDir, "${AUTO_BACKUP_FILE_PREFIX}${day}.zip")
        val tmpFile = File(metaagentDir, "${targetFile.name}.tmp")
    ...
    private suspend fun createManualBackup(context: Context): File {
        ...
        val targetFile = File(metaagentDir, "${MANUAL_BACKUP_FILE_PREFIX}${timestamp}.zip")
        val tmpFile = File(metaagentDir, "${targetFile.name}.tmp")
    ...
    private fun enforceMaxBackupCount(context: Context, keepLatest: Int) {
        ...
        val newDir = MetaAgentBackupDirs.roomDbDir()
