package com.ai.assistance.metaagent.data.repository

import android.content.Context
import androidx.compose.ui.graphics.Color
import com.ai.assistance.metaagent.R
import com.ai.assistance.metaagent.data.db.ObjectBoxManager
import com.ai.assistance.metaagent.data.model.Memory
import com.ai.assistance.metaagent.data.model.MemoryLink
import com.ai.assistance.metaagent.data.model.MemoryTag
import com.ai.assistance.metaagent.data.model.MemoryTag_
import com.ai.assistance.metaagent.data.model.Memory_
import com.ai.assistance.metaagent.data.model.DocumentChunk
import com.ai.assistance.metaagent.data.model.Embedding
import com.ai.assistance.metaagent.data.model.CloudEmbeddingConfig
import com.ai.assistance.metaagent.data.model.DimensionCount
import com.ai.assistance.metaagent.data.model.EmbeddingDimensionUsage
import com.ai.assistance.metaagent.data.model.EmbeddingRebuildProgress
import com.ai.assistance.metaagent.data.preferences.MemorySearchSettingsPreferences
import com.ai.assistance.metaagent.services.CloudEmbeddingService
import com.ai.assistance.metaagent.ui.features.memory.screens.graph.model.Edge
import com.ai.assistance.metaagent.ui.features.memory.screens.graph.model.Graph
import com.ai.assistance.metaagent.ui.features.memory.screens.graph.model.Node
...
import com.ai.assistance.metaagent.data.model.MemoryExportData
import com.ai.assistance.metaagent.data.model.SerializableMemory
import com.ai.assistance.metaagent.data.model.SerializableLink
import com.ai.assistance.metaagent.data.model.ImportStrategy
import com.ai.assistance.metaagent.data.model.MemoryImportResult
import com.ai.assistance.metaagent.data.model.MemorySearchDebugCandidate
import com.ai.assistance.metaagent.data.model.MemorySearchDebugInfo
...
class MemoryRepository(private val context: Context, profileId: String) {
        ...
        if (memory.isDocumentNode) {
            if (memory.chunkIndexFilePath != null) {
                try {
                    ...
                    if (indexFile.exists()) {
                        if (indexFile.delete()) {
                            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Deleted chunk index file: ${indexFile.path}")
                        } else {
                            com.ai.assistance.metaagent.util.AppLogger.w("MemoryRepo", "Failed to delete chunk index file: ${indexFile.path}")
                        }
                    }
                } catch (e: Exception) {
                    com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Error deleting chunk index file for memory ID $memoryId", e)
                }
            }
            // 删除关联的区块
            ...
            if (chunkIds.isNotEmpty()) {
                ...
                com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Deleted ${chunkIds.size} associated chunks for document.")
            }
        }
    ...
    private fun cleanupDanglingLinksIfNeeded(force: Boolean = false) {
        ...
        if (danglingLinkIds.isNotEmpty()) {
            ...
            com.ai.assistance.metaagent.util.AppLogger.w(
                "MemoryRepo",
    ...
    suspend fun linkMemories(
        ...
        if (existingLink != null) {
            // 链接已存在，可以选择更新或直接返回
            // 这里我们选择直接返回，不创建重复链接
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Link already exists from memory ${source.id} to ${target.id} with type $type")
            return@withContext
        }
    ...
    private suspend fun runSearchMemoriesWithDebug(
        ...
        if (memoriesToSearch.isEmpty()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "No memories found in folder '$folderPath' to search.")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = buildDebug(
                    keywords = keywords,
        ...
        if (reverseContainmentResults.isNotEmpty()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Reverse containment: ${reverseContainmentResults.size} matches")
        }
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "--- Starting Semantic Search for ${keywords.size} keywords ---")
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.ai.assistance.metaagent.util.AppLogger.w("MemoryRepo", "Failed to generate embedding for: '$keyword'")
                    return@forEach
                }
            ...
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "--- Semantic Search Completed ---")
        }
        ...
        if (edgesTraversed > 0) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Graph expansion: ${edgesTraversed} edges traversed")
        }
        ...
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Final results: ${filteredScores.size}/${scores.size} above threshold")
        ...
        sortedScoresForLogging.take(3).forEach { (id, score) ->
            ...
            if (memory != null) {
                com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "  Top: [${memory.title}] = ${String.format("%.4f", score)}")
            }
        }
        ...
        if (filteredScores.isEmpty()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "No memories above relevance threshold")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = debugInfo
            )
    ...
    suspend fun searchChunksInDocument(memoryId: Long, query: String): List<DocumentChunk> = withContext(Dispatchers.IO) {
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "--- Starting search in document (Memory ID: $memoryId) for query: '$query' ---")
        val memory = findMemoryById(memoryId) ?: return@withContext emptyList<DocumentChunk>().also {
            com.ai.assistance.metaagent.util.AppLogger.w("MemoryRepo", "Document with ID $memoryId not found.")
        }
        if (!memory.isDocumentNode) {
        ...
        if (query.isBlank()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Query is blank, returning all chunks sorted by index.")
        ...
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Combined and deduplicated results count: ${combinedResults.size}. Results are now ordered by relevance.")
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "--- Search in document finished ---")
    ...
    private fun cleanupLegacyVectorIndexFiles() {
        val vectorDir = MetaAgentPaths.vectorIndexDir(context)
        if (vectorDir.exists()) {
    ...
    suspend fun getAllFolderPaths(): List<String> = withContext(Dispatchers.IO) {
        ...
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Total memories: ${allMemories.size}")
        val folderPaths = allMemories
            .map { normalizeFolderPath(it.folderPath) ?: context.getString(R.string.memory_uncategorized) }
            .distinct()
            .sorted()
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Unique folders: $folderPaths")
        folderPaths
    }
    ...
    suspend fun renameFolder(oldPath: String, newPath: String): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Failed to rename folder", e)
            false
        }
    }
    ...
    suspend fun moveMemoriesToFolder(memoryIds: List<Long>, targetFolderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Failed to move memories", e)
            false
        }
    }
    ...
    suspend fun createFolder(folderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Failed to create folder", e)
            false
        }
            }
    ...
    suspend fun mergeMemories(
        ...
        if (sourceMemories.size < 2) {
            com.ai.assistance.metaagent.util.AppLogger.w("MemoryRepo", "Merge requires at least two unique source memories to be found. Found: ${sourceMemories.size} from titles: ${sourceTitles.joinToString()}.")
        ...
        try {
            ...
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Error during memory merge transaction.", e)
            return@withContext null
        }
    ...
    suspend fun deleteMemoriesByUuids(uuids: Set<String>): Boolean = withContext(Dispatchers.IO) {
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Attempting to delete memories with UUIDs: $uuids")
        if (uuids.isEmpty()) {
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "UUID set is empty, nothing to delete.")
        ...
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Found ${memoriesToDelete.size} memories to delete.")
        ...
        try {
            store.runInTx {
                ...
                com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Found ${linkIdsToDelete.size} unique links and ${chunkIdsToDelete.size} chunks to delete.")

                // 2. 批量删除链接和区块
                if (linkIdsToDelete.isNotEmpty()) {
                    linkBox.removeByIds(linkIdsToDelete)
                    com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${linkIdsToDelete.size} links.")
                }
                if (chunkIdsToDelete.isNotEmpty()) {
                    chunkBox.removeByIds(chunkIdsToDelete)
                    com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${chunkIdsToDelete.size} chunks.")
                }

                // 3. 批量删除记忆本身
                memoryBox.removeByIds(memoryIdsToDelete.toList())
                com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${memoriesToDelete.size} memories.")
            }
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Transaction completed successfully.")
        } catch (e: Exception) {
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Error during bulk delete transaction.", e)
        ...
        for (memory in memoriesToDelete) {
            ...
            if (memory.isDocumentNode && memory.chunkIndexFilePath != null) {
                try {
                    ...
                    com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Error deleting chunk index file for memory UUID ${memory.uuid}", e)
                }
            }
        }
        com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Removed memories from vector index and cleaned up chunk index files.")
        ...
        memories.forEach { memory ->
            ...
            memory.links.forEach { link ->
                ...
                if (sourceId != null &&
                    ...
                    // com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Discarding edge: $sourceId -> $targetId
    ...
    suspend fun deleteFolder(folderPath: String) {
        withContext(Dispatchers.IO) {
            ...
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Deleted folder '$folderPath', moved ${memories.size} memories to uncategorized")
        }
    }
    ...
    suspend fun importMemoriesFromJson(
        ...
        try {
            ...
            com.ai.assistance.metaagent.util.AppLogger.d("MemoryRepo", "Import completed: $newCount new, $updatedCount updated, $skippedCount skipped, $newLinksCount links")
            ...
            com.ai.assistance.metaagent.util.AppLogger.e("MemoryRepo", "Failed to import memories", e)
            throw e
        }
    }
