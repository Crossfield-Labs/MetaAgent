    ...
    return """
        (function() {
            ...
            function getActiveExports() {
                return typeof root.__metaagentGetActiveModuleExports === 'function'
                    ? root.__metaagentGetActiveModuleExports()
                    : null;
            }
            ...
            function buildInlineFunctionName(definition) {
                ...
                return '__metaagent_inline_hook_' + safeId + '_' + inlineHookCounter;
            }
            ...
            function normalizeScreenField(definition, label) {
                ...
                } else if (typeof screen === 'function' && typeof screen.__metaagent_toolpkg_module_path === 'string') {
                    path = screen.__metaagent_toolpkg_module_path.trim().replace(/\\/g, '/');
                } else if (
                    screen &&
                    typeof screen === 'object' &&
                    typeof screen.default === 'function' &&
                    typeof screen.default.__metaagent_toolpkg_module_path === 'string'
                ) {
                    path = screen.default.__metaagent_toolpkg_module_path.trim().replace(/\\/g, '/');
                }
                if (!path) {
            ...
            function resolveCurrentToolPkgTarget() {
                var callId = String(root.__metaagentCurrentCallId || '').trim();
                var callState =
                    callId && typeof root.__metaagentGetCallState === 'function'
                ...
                var candidates = [
                    params.__metaagent_ui_package_name,
                    params.toolPkgId,
                    params.containerPackageName,
                    params.__metaagent_toolpkg_subpackage_id,
                    params.__metaagent_package_name
                ];
