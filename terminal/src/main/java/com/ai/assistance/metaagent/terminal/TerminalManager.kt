...
import com.ai.assistance.metaagent.terminal.data.TerminalState
import com.ai.assistance.metaagent.terminal.data.CommandHistoryItem
import com.ai.assistance.metaagent.terminal.data.QueuedCommand
import com.ai.assistance.metaagent.terminal.view.domain.OutputProcessor
...
import com.ai.assistance.metaagent.terminal.data.SessionInitState
...
import com.ai.assistance.metaagent.terminal.utils.SSHDServerManager
import com.ai.assistance.metaagent.terminal.provider.filesystem.FileSystemProvider
import com.ai.assistance.metaagent.terminal.provider.filesystem.LocalFileSystemProvider
import com.ai.assistance.metaagent.terminal.provider.filesystem.PRootMountMapping
import com.ai.assistance.metaagent.terminal.provider.type.HiddenExecResult
import com.ai.assistance.metaagent.terminal.provider.type.TerminalProvider
import com.ai.assistance.metaagent.terminal.provider.type.TerminalType
import com.ai.assistance.metaagent.terminal.provider.type.LocalTerminalProvider
import com.ai.assistance.metaagent.terminal.provider.type.SSHTerminalProvider
import com.ai.assistance.metaagent.terminal.data.TerminalSessionData
import com.ai.assistance.metaagent.terminal.view.domain.ansi.AnsiTerminalEmulator
...
class TerminalManager private constructor(
    ...
    suspend fun createNewSession(
        ...
        val success = withTimeoutOrNull(30000) { // 30秒超时
            terminalState.first { state ->
                ...
                session?.initState == com.ai.assistance.metaagent.terminal.data.SessionInitState.READY
            }
        }
    ...
    private fun extractAssets() {
        try {
            val assets = listOf("setup_fake_sysdata.sh")
    ...
    @Throws(IOException::class)
    private fun ensureUbuntuArchiveAvailable() {
        val archiveFile = File(filesDir, UBUNTU_FILENAME)
        if (archiveFile.exists() && archiveFile.length() > 0L) {
            Log.d(TAG, "Ubuntu runtime archive already exists at ${archiveFile.absolutePath}")
            return
        }
...
EOF
    ...
    private fun handleRegularCommand(command: String, session: com.ai.assistance.metaagent.terminal.data.TerminalSessionData, commandId: String) {
